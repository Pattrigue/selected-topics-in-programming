...
TEST_CASE("type_name")
{
	...
	SUBCASE("auto test")
	{
		...
		CHECK(res);
		CHECK(type_name<decltype(v4)>() == "std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >"); // RTTI gives implementation specific type
		CHECK(type_name<decltype(v5)>() == "std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >"); // RTTI gives implementation specific type
		res = type_name<decltype(v6)>() == "int*";
		CHECK(res);
		res = type_name<decltype(v7)>() == "char*"; 
		CHECK(res);
		CHECK(type_name<decltype(v8)>() == "std::vector<int, std::allocator<int> >"); // RTTI gives very detailed type
	}
	SUBCASE("hand-crafted")
	{
		bool res;
		res = type_name<size_t>() == "unsigned long long"; 
		CHECK(res);
		...
		CHECK(res);
		CHECK(type_name<std::string>() == "std::__cxx11::basic_string<char, std::char_traits<char>, std::allocator<char> >"); // RTTI gives implementation specific type
		CHECK(type_name<std::string_view>() == "std::basic_string_view<char, std::char_traits<char> >"); // RTTI gives very detailed type
		res = type_name<decltype(f)>() == "int volatile& (*)(int const&)";
		CHECK(res);
		res = type_name<std::function<const char*(const int&)>>() == "std::function<char const* (int const&)>"; 
		CHECK(res);
		res = type_name<std::function<double(const int&)>>() == "std::function<double (int const&)>"; 
		CHECK(res);
